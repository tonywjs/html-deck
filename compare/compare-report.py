...
import json, pathlib, html, base64, sys
...
<td class="cov">{"".join(cov)}</td></tr>''')
    ...
    for sp in shots:
        ...
        if jp.exists():
            imgs.append('data:image/jpeg;base64,' + base64.b64encode(jp.read_bytes()).decode())
        else:
            imgs.append(sp)
    cards.append(f'<section><h2>{names[k]} <a href="{k}/deck.html">열기</a></h2><div class="strip">'
                 + ''.join(f'<img src="{u}" loading="lazy">' for u in imgs) + '</div></section>')
...
section{{margin-top:28px}} h2{{font-size:17px}} h2 a{{font-size:13px;margin-left:8px}} .strip{{display:flex;gap:8px;overflow-x:auto}} .strip img{{height:300px;border:1px solid #dfe3ea;border-radius:8px;background:#fff}}
</style></head><body><h1>html-deck 편집기 사용법 덱: 모델 비교</h1>
<p>같은 브리프(스킬 SKILL.md + 동일 지시문)로 각 모델이 만든 deck.html. 검사는 전부 같은 하네스로 수행했다. Claude 세 모델은 공유 브라우저 충돌을 막으려 브라우저 없이 설계했고 GPT 네 모델은 스스로 브라우저 검증을 했지만, 표의 판정(품질 스니펫, 콘솔, 장면 진행)은 일곱 모두 이 하네스가 똑같이 수행한 값이다. 품질 스니펫은 SKILL.md 기준대로 글자 하한 14px은 전 슬라이드에, 채움 70%는 타이틀을 뺀 콘텐츠 슬라이드에 적용한다.</p>
